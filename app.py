...
if __name__ == "__main__":
    # Tells Flask to listen on all available network interfaces, allowing my app to be
    # accessible  not only my local machine but also other devices
    app.run(host='0.0.0.0', port=5000, debug=True)

    # In this case Flask runs by default on Host -> 127.0.0.1 and Port -> 5000
    # meaning that the app is only accessible from my local machine
    #app.run(debug=True)